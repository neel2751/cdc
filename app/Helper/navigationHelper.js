...
import { initFacebookPixel } from "./facebookHelper";
...
export function NavigationEvents({ children }) {
  ...
  const handleClick = () => {
    // this is one for cookies model store the value in the  browser
    // this is one for local storage model store the value in the browser
    // this is one for session storage model store the value in the browser
    localStorage.setItem("close", true); // this is one for cookies model store the value in the  browser
    setClose(true); // close the modal
  };

  useEffect(() => {
    const check = localStorage.getItem("close");
    if (check) {
      setClose(check);
    } else {
      setClose(false);
    ...
    if (process.env.NEXT_PUBLIC_MODE === "Production") {
      async function storePageData() {
        ...
        // await storePageView(path);
      }
      // storePageData();
    }
    // console.clear();
    // console.log("Pixel is running");
    // initFacebookPixel(process.env.NEXT_PUBLIC_FB_PIXEL_ID || 1220267655616224);
    // initFacebookPixel("1220267655616224");
  ...
  return (
    <>
      <div
        ...
        className={`fixed ${
          close ? "hidden" : "block"
        } bottom-0 end-0 z-[60] sm:max-w-md w-full mx-auto p-6`}
      >
          ...
          <div className="flex gap-x-4">
            ...
            <div>
              <button
                onClick={handleClick}
                type="button"
                className="p-2 inline-flex items-center gap-x-2 text-sm font-semibold rounded-full border border-transparent text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none"
